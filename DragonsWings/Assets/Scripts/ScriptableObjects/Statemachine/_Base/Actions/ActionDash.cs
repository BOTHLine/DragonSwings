...
// [CreateAssetMenu(menuName = "Statemachine/Actions/Dash")]
public class ActionDash // : Action
{
    ...
    float distanceCovered = (Time.time - startTime) * dashSpeed.Get(controller.gameObject);
    ...
    public override void EnterState(StateController controller)
    {
        ...
        trueDashDistance = Mathf.Min(dashRange.Value, totalDashDistance);
    ...
    private bool ReachedDestination(StateController controller) { return ((Vector2)controller.transform.position - start).sqrMagnitude + distanceThreshold.Get(controller.gameObject) >= trueDashDistance * trueDashDistance; }
    */
}