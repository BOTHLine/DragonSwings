...
{
    public static Quaternion GetLookAtRotation2D(Vector2 direction, float rotation = 0.0f)
    {
        float zRotation = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg;
        return Quaternion.Euler(0.0f, 0.0f, zRotation + rotation);
    }

    public static Quaternion GetLookAtRotation2D(Vector2 fromPosition, Vector2 toPosition, float rotation = 0.0f)
    { return GetLookAtRotation2D(toPosition - fromPosition, rotation); }
